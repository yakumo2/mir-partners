...
const REGISTRATION_CAP = 20;
const REGISTRATION_REWARD = 300;
const LEVEL50_CAP = 50;
const LEVEL50_REWARD = 500;
const STREAMING_REWARD_MAP: Record<number, number> = {
  0: 0,
  10: 50_000,
  30: 150_000,
  50: 300_000,
...
function computeMonthResults(
  ...
  return months.reduce<MonthResult[]>((acc, month, index) => {
    ...
    const cappedRegistrations = Math.min(month.registrations ?? 0, REGISTRATION_CAP);
    const cappedLevel50 = Math.min(month.level50 ?? 0, Math.min(LEVEL50_CAP, cappedRegistrations));
    const registrationReward = cappedRegistrations * REGISTRATION_REWARD;
    const level50Reward = cappedLevel50 * LEVEL50_REWARD;
    const specialBonus = specialBonuses[index] ?? 0;
    const bonusMira = registrationReward + level50Reward + specialBonus;
    const monthMira = selfMira + teamMira + bonusMira;
    ...
    if (registrationReward > 0) {
      miraSteps.push(
        `注册奖励：${formatNumber(cappedRegistrations)} 人 × ${REGISTRATION_REWARD} = ${formatNumber(registrationReward)} 米拉`
      );
    }
    if (level50Reward > 0) {
      miraSteps.push(
        `50 级奖励：${formatNumber(cappedLevel50)} 人 × ${LEVEL50_REWARD} = ${formatNumber(level50Reward)} 米拉`
      );
    }
    if (specialBonus > 0) {
      miraSteps.push(`专项激励奖励：${formatNumber(specialBonus)} 米拉`);
    }
    miraSteps.push(
      `当月新增米拉 = ${formatNumber(selfMira)} + ${formatNumber(teamMira)}${
        bonusMira > 0 ? ` + ${formatNumber(bonusMira)}` : ""
      } = ${formatNumber(monthMira)} 米拉`
    );
    miraSteps.push(
...
export function SimulationCore(props: SimulationProps = {}) {
  const { externalGrowth, showGrowthInputs = false } = props;
  const [monthInputs, setMonthInputs] = useState<MonthInput[]>(() =>
    defaultMonths.map((month) => ({ ...month }))
  );
  ...
  const [streamingHours, setStreamingHours] = useState<number>(0);
  const [isDidi, setIsDidi] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (!externalGrowth) return;
    setMonthInputs((previous) =>
      previous.map((month, index) => {
        const growth = externalGrowth[index];
        if (!growth) return month;
        const registrations = Math.max(0, growth.registrations || 0);
        const level50 = Math.max(0, Math.min(growth.level50 || 0, registrations));
        if (month.registrations === registrations && month.level50 === level50) {
          return month;
        }
        return {
          ...month,
  ...
  const specialBonusFirstMonth = useMemo(() => {
    const streamingBonus = STREAMING_REWARD_MAP[streamingHours] ?? 0;
    const didiBonus = isDidi ? 500_000 : 0;
    const courierBonus = isCourier ? 500_000 : 0;
    return streamingBonus + didiBonus + courierBonus;
  }, [streamingHours, isDidi, isCourier]);

  const specialBonusVector = useMemo(
    () => monthInputs.map((_, idx) => (idx === 0 ? specialBonusFirstMonth : 0)),
    [monthInputs, specialBonusFirstMonth]
  );
  ...
  const monthResults = useMemo(
    ...
    [monthInputs, tiers, distribution.upline, specialBonusVector]
  );
  ...
  const handleChange = (index: number, field: keyof MonthInput, value: string) => {
    setMonthInputs((previous) => {
      const next = [...previous];
      ...
      if (field === "registrations" && next[index].level50 > safeValue) {
        next[index].level50 = safeValue;
      }
      if (field === "level50" && safeValue > next[index].registrations) {
        next[index].level50 = next[index].registrations;
      }
      return next;
    });
  };
  ...
  const renderSpecialControls = () => {
    if (!showGrowthInputs) return null;
    return (
      <section className={styles.sideSection}>
        <h2>专项激励</h2>
        <div className={styles.inputField}>
          <label htmlFor="streaming-hours">抖音直播时长</label>
        ...
        <div className={styles.inlineRadios}>
          ...
          <div className={styles.radioGroup}>
            <button
              type="button"
              className={`${styles.radioButton} ${isDidi ? styles.radioActive : ""}`}
              onClick={() => setIsDidi(true)}
            >
              是
            </button>
            <button
        ...
        <div className={styles.inlineRadios}>
          ...
          <div className={styles.radioGroup}>
            <button
              type="button"
              className={`${styles.radioButton} ${isCourier ? styles.radioActive : ""}`}
        ...
        <aside className={styles.sidebar}>
          <section className={styles.sideSection}>
            ...
            <p className={styles.sectionLead}>
              调整每个月的充值与下线成长数据，实时查看米拉累计、星级变化与收益结算。
            </p>
                        ...
                        <div className={styles.inlineInputsRow}>
                          <div className={styles.inlineControl}>
                            ...
                            <input
                              ...
                              inputMode="numeric"
                              pattern="[0-9,]*"
                              value={formatNumber(month.registrations)}
                              onChange={(event) => {
                                const raw = event.target.value.replace(/,/g, "");
                                handleChange(index, "registrations", raw);
                              }}
                              onBlur={(event) => {
                                const raw = event.target.value.replace(/,/g, "");
                                handleChange(index, "registrations", raw);
                          ...
                          <div className={styles.inlineControl}>
                            ...
                            <input
                              ...
                              inputMode="numeric"
                              pattern="[0-9,]*"
                              value={formatNumber(month.level50)}
                              onChange={(event) => {
                                const raw = event.target.value.replace(/,/g, "");
                                handleChange(index, "level50", raw);
                              }}
                              onBlur={(event) => {
                                const raw = event.target.value.replace(/,/g, "");
                                handleChange(index, "level50", raw);
          ...
          <section className={styles.sideSection}>
            ...
            <ul className={styles.assumptionList}>
              ...
              {showGrowthInputs ? (
                <li>
                  注册奖励每月最多计 20 人 × 300 米拉；达 50 级奖励最多计 50 人 × 500 米拉，超出上限不再产生额外米拉。
                </li>
              ) : null}
            </ul>
          </section>
